...
#if MXNET_USE_MKLDNN == 1
inline bool SupportMKLDNNLayerNorm(const NDArray& input) {
  return ((input.shape().ndim() > 0) &&
         (input.shape().ndim() < 6) &&
         (input.dtype() == mshadow::kFloat32 || input.dtype() == mshadow::kBfloat16));
}

inline bool SupportMKLDNNLayerNormBwd(const NDArray& input) {
  return ((input.shape().ndim() > 0) &&
         (input.shape().ndim() < 6) &&
...
void LayerNormComputeExCPU(const nnvm::NodeAttrs& attrs,
  ...
  if (SupportMKLDNNLayerNorm(inputs[0])) {
...
void LayerNormGradComputeExCPU(const nnvm::NodeAttrs &attrs,
                               ...
                               const std::vector<OpReqType> &req,
                               const std::vector<NDArray> &outputs) {
  if (SupportMKLDNNLayerNormBwd(inputs[0])) {
...
#if MXNET_USE_MKLDNN == 1
  heads.push_back(n->inputs[2]);  // beta - added at the end in case of fallback to non MKLDNN version
#endif
